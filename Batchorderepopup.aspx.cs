...
public partial class Batchorderepopup : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    itemcodesearch();
                    DataSet dsData = Batchpopgrid.DataSource as DataSet;
                    DataTable dtData = dsData.Tables[0];
                    lblpge.Text = "Page" + "  " + Convert.ToString(Batchpopgrid.PageIndex + 1) + "  " + "of" + "  " + Convert.ToString(Batchpopgrid.PageCount) + "  " + "(" + Convert.ToString(dtData.Rows.Count) + "  " + "Items" + ")";
    ...
    {
        searchqry.Batchpreloadpoppupitemcode(Batchpopgrid, txtitemcode.Text.Trim(),location);
        DataSet dsData = Batchpopgrid.DataSource as DataSet;
        DataTable dtData = dsData.Tables[0];
        if (dtData.Rows.Count == 0)
        {
    ...
    {
        searchqry.Batchpreloadpopup(Batchpopgrid, txtdrugcode.Text.Trim(), txtitemname.Text.Trim(), location);
    ...
    {
        searchqry.Batchbrandname(Batchpopgrid, txtbrand.Text.Trim(), location);
    ...
    public void MFRCODE()
    {
        searchqry.Batchmfrcode(Batchpopgrid, txtmfrcode.Text.Trim(), location);
    ...
    protected void btnsearch_Click(object sender, ImageClickEventArgs e)
    {
        ...
        {
            searchqry.mfrcode(Batchpopgrid, "", location);
            DataSet dsData = Batchpopgrid.DataSource as DataSet;
            DataTable dtData = dsData.Tables[0];
            lblpge.Text = "Page" + "  " + Convert.ToString(Batchpopgrid.PageIndex + 1) + "  " + "of" + "  " + Convert.ToString(Batchpopgrid.PageCount) + "  " + "(" + Convert.ToString(dtData.Rows.Count) + "  " + "Items" + ")";
            lblpge.Font.Bold = false;
    ...
    protected void Batchpopgrid_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //{           
        //    int Drgid = (int)Batchpopgrid.DataKeys[e.Row.RowIndex].Value;
    ...
    protected void Batchpopgrid_PageIndexChanging(object sender, GridViewPageEventArgs e)
    {
        searchquery();
        Batchpopgrid.PageIndex = e.NewPageIndex;
        Batchpopgrid.DataBind();
        DataSet dsData = Batchpopgrid.DataSource as DataSet;
        DataTable dtData = dsData.Tables[0];
        lblpge.Text = "Page" + "  " + Convert.ToString(Batchpopgrid.PageIndex + 1) + "  " + "of" + "  " + Convert.ToString(Batchpopgrid.PageCount) + "  " + "(" + Convert.ToString(dtData.Rows.Count) + "  " + "Items" + ")";
        if (dtData.Rows.Count == 0)
    ...
    protected void Batchpopgrid_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Btn")
        {
            int index = Convert.ToInt32(e.CommandArgument);
            GridViewRow selectedRow = Batchpopgrid.Rows[index];
            string packtype = Batchpopgrid.Rows[selectedRow.RowIndex].Cells[5].Text;
            if (packtype.ToLower() != "BOX".ToLower())
            {
                int Drgid = (int)Batchpopgrid.DataKeys[selectedRow.RowIndex].Value;
                ScriptManager.RegisterClientScriptBlock(this.Page, typeof(Page), "popup", "passValueToParent('" + Drgid + "')", true);
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, typeof(Page), "Alert", "<script>alert('Batch Order is only for strips');</script>", false);
            }
    ...
    public void DrugCodeCheck(string DrugCode)
    {
        ...
        using (SqlConnection con = DBCon.getstring())
        {
            string Commt = "Select i.Item_Effective_date_To ,il.Store_Effective_Date_To,u.Status,ba.Status  from Item_user_Master as u left join item_master as i on i.MasterID=u.MasterID left join Brand_Master as b on b.BrandID=u.Brandid left join Brand_Allot as ba on ba.BrandID=b.BrandID and ba.MasterID=i.MasterID left join Pharmacy as p on p.PharmacyID =u.PharmacyID left join Item_Location as IL on IL.MasterID=i.MasterID and IL.PharmacyID=p.PharmacyID left join Packtype_Master as pm on pm.ID=u.PacktypeID where   i.Drug_Code ='" + DrugCode + "'  and p.Location_Name='" + location + "'  and pm.Packtype = 'STRIP' ";
            cmd = new SqlCommand(Commt, con);
    ...
    public void ItemNameCheck(string ItemName)
    {
        ...
        using (SqlConnection con = DBCon.getstring())
        {
            string Commt = "Select i.Item_Effective_date_To ,il.Store_Effective_Date_To,u.Status,ba.Status  from Item_user_Master as u left join item_master as i on i.MasterID=u.MasterID left join Brand_Master as b on b.BrandID=u.Brandid left join Brand_Allot as ba on ba.BrandID=b.BrandID and ba.MasterID=i.MasterID left join Pharmacy as p on p.PharmacyID =u.PharmacyID left join Item_Location as IL on IL.MasterID=i.MasterID and IL.PharmacyID=p.PharmacyID left join Packtype_Master as pm on pm.ID=u.PacktypeID where   i.Item_Name ='" + ItemName + "'  and p.Location_Name='" + location + "'  and pm.Packtype = 'STRIP' ";
            cmd = new SqlCommand(Commt, con);
    ...
    public void BrandNameCheck(string BrandName)
    {
        ...
        using (SqlConnection con = DBCon.getstring())
        {
            string Commt = "Select i.Item_Effective_date_To ,il.Store_Effective_Date_To,u.Status,ba.Status from Item_user_Master as u left join item_master as i on i.MasterID=u.MasterID left join Brand_Master as b on b.BrandID=u.Brandid left join Brand_Allot as ba on ba.BrandID=b.BrandID and ba.MasterID=i.MasterID left join Pharmacy as p on p.PharmacyID =u.PharmacyID left join Item_Location as IL on IL.MasterID=i.MasterID and IL.PharmacyID=p.PharmacyID left join Packtype_Master as pm on pm.ID=u.PacktypeID where  b.brandname like '" + BrandName + "%' and p.Location_Name='" + location + "' and  pm.Packtype = 'STRIP' ";
            cmd = new SqlCommand(Commt, con);
